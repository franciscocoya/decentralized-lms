...
import { register as signup } from "@/service/authService";
...
const SignUpPage = () => {
  ...
  const checkFields = (data: FieldValues) => {
    if (data.password !== data.remember) {
      return tM("passwordsDontMatch");
    }
  };
  ...
  return (
    <>
      <article className="h-full flex flex-col justify-center items-center gap-[20px]">
        <BaseHeading text={"Create an account"} />
            ...
            <form onSubmit={handleSubmit(handleRegister)} className="space-y-5">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      {capitalize(tC("form.input.password"))}
                    </FormLabel>
                    <FormControl>
                      <Input
                        type="password"
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{capitalize(tC("form.input.remember"))}</FormLabel>
                    <FormControl>
                      <Input
                        type="password"
                        {...register("password", { required: true })}
              ...
              <Button type="submit" className="w-full">
                {isLoginProcessing ? tM("loading") : tC("buttons.register")}
              </Button>
            </form>
          </Form>
          {loginErrorMsg && (
